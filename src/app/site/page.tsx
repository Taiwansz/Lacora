    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[.78fr_1.22fr]">
        ...
        <section className="overflow-hidden rounded-3xl border border-border bg-surface shadow-card" aria-label="Prévia do site do casal">
          <div className="flex items-center justify-between border-b border-border bg-[#111B3A] px-4 py-3 text-[#F8F1DF]"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#7A2738]" /><span className="text-[10px] font-semibold uppercase tracking-[.14em]">Prévia ao vivo</span></div><span className="flex items-center gap-1.5 text-[10px] text-[#D0CBC4]"><Link2 className="h-3 w-3" />{publicPath}</span></div>
          <div className="relative flex min-h-[24rem] flex-col items-center justify-center overflow-hidden bg-[#111B3A] px-6 py-14 text-center text-[#F8F1DF]"><div className="absolute inset-0 bg-[url('/brand/lacora-ribbon-pattern.svg')] bg-cover bg-center opacity-60" /><div className="relative"><span className="text-[10px] font-bold uppercase tracking-[.28em] text-[#D7A1AD]">Celebre conosco</span><h2 className="mt-4 font-serif text-5xl font-medium sm:text-6xl">{websiteSettings.title || `${partner1} & ${partner2}`}</h2><div className="mx-auto my-6 h-px w-14 bg-[#A95768]" /><p className="text-xs tracking-[.12em] text-[#DED2C0]">{dateText}</p><p className="mt-2 flex items-center justify-center gap-1.5 text-xs text-[#D0CBC4]"><MapPin className="h-3.5 w-3.5" />{cityText}</p></div></div>
