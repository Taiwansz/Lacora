...
export default function NotificacoesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2"><button type="button" onClick={() => setFilter('todas')} className={filter === 'todas' ? 'rounded-full bg-[#111B3A] px-4 py-2 text-[11px] font-bold text-white' : 'rounded-full border border-border bg-surface px-4 py-2 text-[11px] font-semibold text-[#655B50]'}>Todas ({notifications.length})</button><button type="button" onClick={() => setFilter('nao_lidas')} className={filter === 'nao_lidas' ? 'rounded-full bg-[#111B3A] px-4 py-2 text-[11px] font-bold text-white' : 'rounded-full border border-border bg-surface px-4 py-2 text-[11px] font-semibold text-[#655B50]'}>Não lidas ({unread})</button></div>
