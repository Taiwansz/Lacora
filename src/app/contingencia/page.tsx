...
import { ShieldAlert, Plus, AlertTriangle, Phone } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className="bg-surface rounded-3xl border border-border shadow-card overflow-hidden">
        <div className="p-6 border-b border-border">
          <h2 className="font-serif text-lg font-bold text-charcoal">Matriz de Contingência & Riscos Mapeados</h2>
        </div>
              ...
              {risks.map((rk) => (
                <tr key={rk.id} className="hover:bg-surface-muted/40 transition-colors">
                  ...
                  <td className="p-4 text-slate-600 text-[11px]">
                    ...
                    <span className="text-[10px] text-slate-400">Gatilho: {rk.triggerEvent}</span>
                  </td>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleCreate} className="bg-surface p-6 rounded-3xl border border-border max-w-md w-full shadow-floating space-y-4">
            <h3 className="font-serif text-lg font-bold text-charcoal">Mapear Risco no Plano de Contingência</h3>
            <div>
              <label className="block text-xs font-semibold text-charcoal mb-1">Descrição do Risco</label>
              <input
                type="text"
                required
                ...
                placeholder="Ex: Atraso na entrega dos doces finos"
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-xs font-semibold text-charcoal mb-1">Responsável</label>
                <input
                  type="text"
                  required
            ...
            <div>
              <label className="block text-xs font-semibold text-charcoal mb-1">Plano Preventivo</label>
              <textarea
                required
                ...
                placeholder="O que fazer antes para evitar o risco..."
              />
            </div>
            <div>
              <label className="block text-xs font-semibold text-charcoal mb-1">Plano de Resposta (Se ocorrer)</label>
              <textarea
                required
                value={newRisk.responsePlan}
                onChange={(e) => setNewRisk({ ...newRisk, responsePlan: e.target.value })}
                className="w-full text-xs p-2.5 border border-border rounded-xl outline-none h-16"
                placeholder="Ação imediata caso o gatilho seja acionado..."
              />
            </div>
